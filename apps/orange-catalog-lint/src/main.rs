...
//! The tool that guards the Orange Catalog repository.
//!
//! Every rule it applies lives in `orange-catalog`, which is also what the
...
use orange_catalog::{Authorization, Index, Owners, Severity, authorize, scan, validate};
...
#[derive(Parser)]
#[command(
    name = "orange-catalog-lint",
    about = "Validates an Orange Catalog checkout and generates its index",
    version
)]
struct Cli {
    #[command(subcommand)]
