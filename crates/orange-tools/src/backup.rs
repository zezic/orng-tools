...
use crate::{BuildId, Error, Installation, OrangeHome, Result, fs};
...
impl Backup {
    ...
    pub fn location(home: &OrangeHome, build: &BuildId) -> Self {
    ...
    pub fn list(home: &OrangeHome) -> Result<Vec<Backup>> {
...
mod tests {
    ...
    fn a_backup_is_named_for_the_build_it_came_from() {
        let home = OrangeHome::at(Path::new("/tmp/orange-test"));
    ...
    fn listing_a_home_that_has_never_been_used_is_empty_not_an_error() {
        let home = OrangeHome::at(Path::new("/nonexistent/orange"));
